...
import { Can } from "../Can";
import { AvailabilityChip } from "../AvailabilityStatus";
        ...
        try {
          ...
          setOptions(
            [...data.users].sort((left, right) => {
              const leftAvailable =
                left.availabilityStatus === "available" ? 0 : 1;
              const rightAvailable =
                right.availabilityStatus === "available" ? 0 : 1;
              return (
  ...
  const handleSaveTicket = async (e) => {
    ...
    try {
      ...
      if (selectedUser) {
        ...
        if (selectedUser.availabilityStatus !== "available") {
          const confirmed = window.confirm(
            i18n.t("transferTicketModal.unavailableWarning"),
          );
          if (!confirmed) {
            setLoading(false);
            return;
          }
          data.forceAssignUnavailable = true;
        }
      }
      ...
      if (selectedWhatsapp && loggedInUser.profile === "admin") {
        data.whatsappId = selectedWhatsapp;
      }
      data.ecosystemId = selectedEcosystem || null;
      ...
      <form onSubmit={handleSaveTicket}>
        ...
        <DialogContent dividers>
          <Autocomplete
            ...
            renderOption={(option) => (
              ...
              >
                ...
                <AvailabilityChip status={option.availabilityStatus} />
              </div>
            )}
          ...
          <FormControl
            ...
            style={{ marginTop: 20 }}
          >
            <InputLabel>Ecosistema Techkepper</InputLabel>
            <Select
              value={selectedEcosystem}
              onChange={(e) => setSelectedEcosystem(e.target.value)}
              label="Ecosistema Techkepper"
            >
