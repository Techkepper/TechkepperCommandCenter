...
_addSslConfig() {
    ...
    if [ -f ${SSL_CERTIFICATE} ] && [ -f ${SSL_CERTIFICATE_KEY} ]; then
        ...
        grep -qF 'include include.d/ssl-redirect.conf;' ${FILE_SSL_CONF} \
            || echo 'include include.d/ssl-redirect.conf;' >> ${FILE_SSL_CONF};
        grep -qF 'include "include.d/ssl.conf";' ${FILE_CONF} \
            || echo 'include "include.d/ssl.conf";' >> ${FILE_CONF};
        grep -qF "ssl_certificate ${SSL_CERTIFICATE};" ${FILE_CONF} \
            || echo "ssl_certificate ${SSL_CERTIFICATE};" >> ${FILE_CONF};
        grep -qF "ssl_certificate_key ${SSL_CERTIFICATE_KEY};" ${FILE_CONF} \
