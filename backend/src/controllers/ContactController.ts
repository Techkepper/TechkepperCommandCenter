...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  await EmitContactEvent("create", contact);
...
export const update = async (
  ...
  if (req.user.profile !== "admin") {
    ...
    const accessibleTicket = await Ticket.findOne({
      where: {
        [Op.and]: [scope, { contactId: Number(req.params.contactId) }]
      }
    });
    if (!accessibleTicket) {
      throw new AppError("ERR_NO_PERMISSION", 403);
  ...
  await EmitContactEvent("update", contact);
...
export const remove = async (
  ...
  await EmitContactEvent("delete", undefined, Number(contactId));
